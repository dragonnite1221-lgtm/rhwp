...
impl DocumentCore {
    ...
    pub fn paste_hwpx_fragment_in_document_native(
        ...
        if section_idx < self.composed.len() {
            ...
            self.composed
                .resize_with(section_idx + 1, Default::default);
            self.composed[section_idx] = new_composed;
        }
...
<hh:borderFillList></hh:borderFillList>\
    ...
    /// 회귀 테스트 — yangsik 양식부품(표 기반) paste 후 `composed[section_idx]` 가 IR 과
    ...
    #[test]
    fn yangsik_table_paste_syncs_composed() {
        ...
        let frag_path = std::path::PathBuf::from(&home)
            .join("rhwp-layout-profiles/personal-templates/yangsik-fragments/표1.fragment.xml");
        if !frag_path.is_file() {
            eprintln!("[skip] 표1 fragment not found: {}", frag_path.display());
            return;
        }
        let fragment = std::fs::read_to_string(&frag_path).expect("frag read");

        // 표1 의 source_definitions (manifest.json 기준 — 본 test 의 ground truth)
        // 참조: ~/rhwp-layout-profiles/personal-templates/yangsik-fragments/manifest.json
        let source = SourceDefinitions {
