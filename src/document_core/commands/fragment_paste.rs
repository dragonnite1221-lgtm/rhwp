...
fn split_definitions<'a>(
    ...
    let mut pos = 0usize;
    while pos < bytes.len() {
        let Some(rel) = src[pos..].find(&open_prefix) else { break };
        let start = pos + rel;
...
fn max_existing_id(header_xml: &str, tag: &str) -> u32 {
    ...
    while let Some(rel) = header_xml[pos..].find(&open_prefix) {
        let start = pos + rel;
        let after = header_xml.as_bytes().get(start + open_prefix.len()).copied();
        if !matches!(
            after,
            Some(b' ') | Some(b'/') | Some(b'>') | Some(b'\t') | Some(b'\n')
        ) {
...
fn parse_rows(tbl_slice: &str) -> Result<Vec<RowInfo>, FragmentPasteError> {
    ...
    for (rs, re) in row_spans {
        ...
        let cell_spans = find_direct_children(row_slice, "<hp:tr", "</hp:tr>", "<hp:tc", "</hp:tc>")?;
...
/// 모든 행의 rowSpan으로 점유될 (row_idx → 점유된 column 집합)을 사전 계산.
fn build_occupied_sets(rows: &[RowInfo]) -> Vec<std::collections::HashSet<u32>> {
    let row_count = rows.len();
    let mut occupied: Vec<std::collections::HashSet<u32>> =
        vec![std::collections::HashSet::new(); row_count];
    for (r, row) in rows.iter().enumerate() {
        let mut logical_col: u32 = 0;
        for cell in &row.cells {
            // 이미 위에서 점유된 col은 skip
            while occupied[r].contains(&logical_col) {
                logical_col += 1;
            }
            // 이 셀이 차지하는 모든 (row_offset, col_offset) 점유
            for span_r in 0..cell.row_span {
                let target_row = r + span_r as usize;
...
<hh:borderFillList>{border_fills}</hh:borderFillList>\
    ...
    fn id_remap_existing_definition_reuses_id() {
        ...
        let source = SourceDefinitions {
            char_prs:
                "<hh:charPr id=\"9\" height=\"1000\"><hh:fontRef hangul=\"0\"/></hh:charPr>"
                    .into(),
            ..Default::default()
        };
    ...
    fn id_remap_byte_exact_insertion() {
        ...
        assert_eq!(&header[..baseline_close_pos], &baseline[..baseline_close_pos]);
    }
    ...
    fn paste_paragraphs_id_remapped_into_header() {
        ...
        let fragment = r#"<hp:p paraPrIDRef="0"><hp:run charPrIDRef="99"><hp:t>x</hp:t></hp:run></hp:p>"#;
        let source = SourceDefinitions {
    ...
    fn paste_paragraphs_validates_input_unclosed_p() {
        ...
        // 닫히지 않은 <hp:p>
        let fragment =
            r#"<hp:p paraPrIDRef="0"><hp:run charPrIDRef="0"><hp:t>x</hp:t></hp:run>"#;
        let source = SourceDefinitions::default();
        let err = paste_paragraphs_into_section(&section, &mut header, 0, fragment, &source)
            .unwrap_err();
    ...
    fn paste_paragraphs_after_para_idx_out_of_range() {
        ...
        let fragment =
            r#"<hp:p paraPrIDRef="0"><hp:run charPrIDRef="0"><hp:t>x</hp:t></hp:run></hp:p>"#;
        let source = SourceDefinitions::default();
        let err = paste_paragraphs_into_section(&section, &mut header, 5, fragment, &source)
            .unwrap_err();
        assert!(matches!(err, FragmentPasteError::MalformedFragment(_)));
    }
    ...
    fn make_simple_table(rows: u32, cols: u32) -> String {
        let mut out = format!(
            "<hp:tbl rowCnt=\"0\" colCnt=\"{cols}\"><hp:cellzoneList/>"
        );
        for r in 0..rows {
            out.push_str("<hp:tr>");
            for c in 0..cols {
