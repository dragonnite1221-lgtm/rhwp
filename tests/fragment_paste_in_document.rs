...
fn locate_yangsik_hwpx() -> Option<std::path::PathBuf> {
    for var in ["RHWP_TEST_HWPX", "YANGSIK_HWPX_PATH"] {
...
fn integration_paste_paragraphs_in_loaded_hwpx() {
    let Some(sample) = locate_yangsik_hwpx() else {
        eprintln!("HWPX 샘플 없음 — 통합 테스트 스킵");
        return;
    };
    let bytes = fs::read(&sample).expect("read sample");
    let mut doc = DocumentCore::from_bytes(&bytes).expect("from_bytes");

    let raw_section_before_len = doc.get_source_section_xml(0)
        .expect("section 0 raw")
        .len();
    ...
    let result = doc.paste_hwpx_fragment_in_document_native(0, 0, fragment, &source)
        .expect("paste ok");

    assert_eq!(result.inserted_para_count, 1);

    let raw_after = doc.get_source_section_xml(0)
        .expect("section 0 raw after");
...
<hp:tc rowAddr=\"99\" colAddr=\"99\" rowSpan=\"1\" colSpan=\"1\"><hp:subList><hp:p/></hp:subList></hp:tc>\
    ...
    let raw_after = doc.get_source_section_xml(0)
        .expect("section 0 raw after");
    assert!(
        raw_after.contains("rowCnt=\"2\""),
        "rowCnt 99→2 corrected"
    );
    assert!(
...
fn integration_two_consecutive_pastes_reuse_ids() {
    ...
    let r1 = doc.paste_hwpx_fragment_in_document_native(0, 0, fragment, &source)
        .expect("first paste");
    ...
    let r2 = doc.paste_hwpx_fragment_in_document_native(0, 0, fragment, &source)
        .expect("second paste");
